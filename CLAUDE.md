...
**Schema-validation probe (`npm run probe:schema`).** The probe script in `scripts/probe-schema-split.ts` makes one real Sonnet 4.6 call with the production prompt shape (Block A + Block B + minimal Block C, no citations, default voice profile) and validates that the tool_use block returns with all expected metadata fields populated. Run after any change touching `BLOCK_A_TEXT`, `SEND_REPLY_TOOL`, or `RealClaudeClient.sendReplyOnce`. Cost ~$0.003. Note: `cache_creation_input_tokens` is often 0 for probe-shaped prompts because Anthropic's per-segment cache minimum (~1024 tokens for Sonnet) lands above the probe's small Block A + B. Production tenants with full voice profiles + tier-1 facts should clear the threshold; verify with `[brain-cost]` log inspection on a real conversation.
