...
P4r-2 — **RealClaudeClient.sendReply (non-streaming) + retries + cost log.** Implements the SDK call with forced tool-use on `SEND_REPLY_TOOL` (post P4r-3 schema changes — see Gate-1 E), maps `tool_use` block → `SendReplyResult`, handles missing-tool-use refusals (`OUTSIDE_SCOPE` / `MISSING_METADATA`), retry/backoff per Gate-1 K5, `[brain-cost]` log line, conversation-level retry counter cap. Wires `getClaudeClient` to actually return the real client when `ANTHROPIC_API_KEY` is set.

P4r-3 — **Prompt caching wired (`cache_control: ephemeral`).** `tools[]` + end of Block A + end of Block B all marked. Extends `Message.aiMetadata.usage` with `cacheCreationInputTokens` / `cacheReadInputTokens`. `[brain-cache]` block-A SHA log line on first call per process, block-B SHA on first call per tenant — for spotting cache invalidation events. **Tool-use schema restructure (Gate-1 E):** `SEND_REPLY_TOOL` becomes `SEND_REPLY_METADATA_TOOL` carrying only `language`/`groundedness`/`citations_used`/`escalation_recommended`/`escalation_reason`; the `reply` text becomes natural content (streamable token-by-token in P4r-4). `BLOCK_A_TEXT` updated to reference both the natural-content reply and the metadata tool.
